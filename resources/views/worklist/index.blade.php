    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex gap-1 mb-4">
                @foreach(['' => 'All', 'waiting' => 'Waiting', 'in_progress' => 'In Progress', 'completed' => 'Completed', 'cancelled' => 'Cancelled'] as $val => $label)
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($items as $item)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-4 py-3">
                                        @php
                                            $sc = ['waiting' => 'bg-gray-100 text-gray-700', 'in_progress' => 'bg-yellow-100 text-yellow-700', 'completed' => 'bg-green-100 text-green-700', 'cancelled' => 'bg-red-100 text-red-700', 'sent' => 'bg-indigo-100 text-indigo-700', 'failed' => 'bg-red-100 text-red-700'];
                                        @endphp
                                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full {{ $sc[$item->status] ?? 'bg-gray-100 text-gray-700' }}">{{ str_replace('_', ' ', ucfirst($item->status)) }}</span>
                                    </td>
